...
namespace Lab4
{
    class Program
    {
        public static XElement getFileInfo(string path)
        {
            ...
            group f by Path.GetExtension(f).ToLower() into g
            select new
            {
                Type = g.Key,
                Count = g.Count(),
                Size = FormatByteSize(g.Select(file => new FileInfo(file).Length).Sum())
            };
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Enter the name of an input folder (from Desktop):");
            inputFolder = "C:\\Users\\Kyle Pamintuan\\Desktop\\" + Console.ReadLine();

            XDocument report = CreateReport(inputFolder);
            //Console.WriteLine(report.ToString());

            Console.WriteLine("Enter a name for the output file (HTML Report):");
            outputFolder = "C:\\Users\\Kyle Pamintuan\\Desktop\\" + Console.ReadLine();
